...
trait IndicatorCalculator {
  ...
  val request: IndicatorCalculatorRequest
  val name: String
  val db: DatabaseDef
  ...
  def calcOverallByRoute: Map[String, Double] = {
    ...
    request.sample_periods.foreach(period => {
  ...
  def calcOverallBySystem: Double = {
    request.sample_periods.map(period => {
  ...
  def storeIndicators = {
    djangoClient.postIndicators(request.token,
