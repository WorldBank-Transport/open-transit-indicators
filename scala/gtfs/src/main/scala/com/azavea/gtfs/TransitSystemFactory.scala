...
class TransitSystemFactory(records: GtfsRecords) {
  ...
  def forPeriod(start: LocalDateTime, end: LocalDateTime): TransitSystem = {
    val dates = {
              ...
              (for(date <- dates) yield {
                ...
                frequencyRecords.map { frequencyRecord =>
                  frequencyRecord.generateStartTimes(date)
                    .map { startTime =>
                      val offset = stopTimeRecords.head.arrival
                      val scheduledStops = 
                        stopTimeRecords
                          .map { record =>
                          ...
                          .filter { scheduledStop =>
                            start <= scheduledStop.departure && scheduledStop.arrival <= end
                          }
