...
object CompressTrips {
  ...
  //   for (bin <- bins.map(_.sortBy(_.stopTimes.head.arrival.millis))) {
  ...
  //         yield b.stopTimes.head.arrival - a.stopTimes.head.arrival
  //       }.toList
  ...
  //             ret += trip
  //           case Run(x, list) =>
  //             val minTime = list.map(_._2.stopTimes.head.arrival.millis).min
  //             val maxTime = list.map(_._2.stopTimes.head.arrival.millis).max
  ...
  //   val l_start = this.stopTimes.head.arrival
  //   val r_start = that.stopTimes.head.arrival

  //   for ( (l,r) <- stopTimes zip that.stopTimes) {
  //     if (l.stopId != r.stopId) return false
  //     if (l.arrival - l_start != r.arrival - r_start) return false
  //     if (l.departure - l_start != r.departure - r_start) return false
  //   }
  //   true
  // }
}
